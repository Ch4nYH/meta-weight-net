...
def train(train_loader,train_meta_loader,model, vnet,optimizer_a, optimizer_b, optimizer_vnet,epoch):
    print('\nEpoch: %d' % epoch)

    train_backbone_loss = 0
    train_fc_loss = 0
    meta_loss = 0
    ...
    for batch_idx, (inputs, targets) in enumerate(train_loader):
        ...
        meta_model.zero_grad()
        grads = torch.autograd.grad(l_f_meta, (meta_model.fc.params()), create_graph=True)
        meta_lr = args.lr * ((0.1 ** int(epoch >= 80)) * (0.1 ** int(epoch >= 100)))   # For ResNet32
        ...
        train_backbone_loss += loss_backbone.item()
        ...
        if (batch_idx + 1) % 50 == 0:
            print('Epoch: [%d/%d]\t'
                  ...
                  'Prec@1 %.2f\t'
                  'Prec_meta@1 %.2f' % (
                      (epoch + 1), args.epochs, batch_idx + 1, len(train_loader.dataset)/args.batch_size, (train_backbone_loss / (batch_idx + 1)),
                      (train_fc_loss / (batch_idx + 1)), (meta_loss / (batch_idx + 1)), prec_train, prec_meta))
...
def main():
    ...
    for epoch in range(args.epochs):
        adjust_learning_rate(optimizer_model_backbone, epoch)
        adjust_learning_rate(optimizer_model_fc, epoch)
        train(train_loader,train_meta_loader,model, vnet,optimizer_model_backbone, optimizer_model_fc, optimizer_vnet, epoch)
...
def get_backbone(model):
    return model.conv1.params() + model.bn1.params() + model.layer1.params() + \
        model.layer2.params() + model.layer3.params()
        
def get_fc(model):
    return model.linear.params()
